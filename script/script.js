...
function createM() {
    ...
    if (window.navigator.onLine) {
        ...
        marker = new L.marker([person.LAT, person.LON], markerOptions)
            .bindPopup([person.SHIP] + `<br/>` + 'Широта:' + ' ' + [person.LON] + `<br/>` + 'Долгота:' + ' ' + [person.LAT] + `<br/>` + 'Дата:' + ' ' + [person.PERIOD])
            .openPopup()
            .addTo(map);
...
function startLoadTable() {
    ...
    if (window.navigator.onLine) {
        ...
        for (let card of cardData) {
            ...
            if (card.Wave !== null) {
                mWaveHtml = `<span class="anotherTxt">Высота волны</span><span>${card.Wave} м</span>`
            } else {
                mWaveHtml = `N/A`
            }
            if (card.Temp !== null) {
                mTempHtml = `<span class="anotherTxt">Температура</span><span>${card.Temp} °C</span>`
            } else {
                mTempHtml = `N/A`
            }
            if (card.Temp !== null) {
                TempWaterHtml = `<span class="anotherTxt">Температура воды</span><span>${card.TempWater} °C</span>`
            } else {
                TempWaterHtml = `N/A`
            }
            if (card.Speed !== null) {
                mSpeedHtml = `<span class="anotherTxt">Скорость</span><span>${card.Speed} уз</span>`
            } else {
                mSpeedHtml = `N/A`
            }
            if (card.WindSpeed !== null) {
                mWindSpeedHtml = `<span class="anotherTxt">Скорость ветра</span><span>${card.WindSpeed} м/с</span>`
            } else {
                mWindSpeedHtml = `N/A`
            }
            if (card.Course !== null) {
                mCourseHtml = `<span class="anotherTxt">Курс</span><span>${card.Course}</span>`
            } else {
                mCourseHtml = `N/A`
            }
        ...
        for (let cruise of cruiseInfo) {
            ...
            spanArrivalHtml = `<span class="anotherTxt StartWayTxtBlock">Отправление</span><span class="TxtWayBlock">${dataTimDeparture}</span>`
            spanDepartureHtml = `<span class="anotherTxt StartWayTxtBlock">Прибытие</span><span class="TxtWayBlock">${dataTimeArrive}</span>`

            distanceHtml = `<hr style="width: calc(100%*${cruise.Ratio})!important" class="LineDistance">`

            PassedHtml = `<span class="TxtRouteBlock">${cruise.Passed} м. миль</span><span class="TxtRouteBlock">${cruise.PassedTime}</span>`
            StayHtml = `<span class="TxtRouteBlock">${cruise.Stay} м. миль</span><span class="TxtRouteBlock">${cruise.StayTime}</span>`
